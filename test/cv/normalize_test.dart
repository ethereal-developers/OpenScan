...
void main() {
  ...
  group('measureEncodedSizesIsolateEntry', () {
    test('reports the source size and one entry per requested quality',
        () async {
      ...
      expect(sizes['jpg:55']!, lessThan(sizes['jpg:85']!));
      expect(sizes['jpg:85']!, lessThan(sizes['jpg:100']!));
      expect(sizes['png'], isPositive);
    });
    ...
    test('omits png unless asked for it', () async {
      ...
      final sizes = await measureEncodedSizesIsolateEntry(
          {'src': src, 'qualities': [70]});

      expect(sizes.containsKey('png'), isFalse);
      expect(sizes['jpg:70'], isPositive);
    });
  });
}
